...
$charsLen = mb_strlen($chars)-1; //jon 181125c
...
foreach($gridWordsTemp as $g) {
  ...
  for($q=0;$q<mb_strlen($g);$q++) {
   ...
   // print_r($g[$q]);
    //echo("<BR>".$g." , r=$r");// word list ok utf-8
    //$r = $r + 20;
  ...
  $m++;
    ...
    if($_POST['lowerCase'] && $languages[$language]!="Numbers") {
      $ranChar = mb_strtr($chars[$random], ${$languages[$language]}[0], ${$languages[$language]}[1]);
      $ranChar = mb_strtolower($ranChar);
      $gridChar = mb_strtr($grid[$x][$y][0], ${$languages[$language]}[0], ${$languages[$language]}[1]);
      $gridChar = mb_strtolower($gridChar);
    } else {
      $ranChar = mb_substr($chars, $random, 1) ;// $chars[$random]; //Random letter //jon 181125c
	  //echo("<br> chars(random)=".$chars[$random]." random=$random , chars=$chars ,char3=".mb_substr($chars, $random, 1));
