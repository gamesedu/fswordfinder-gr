...
$mainLang = mb_ereg_replace("[@#$%^&*+\\/]","",$_COOKIE['mainLang']);
...
$rows = mb_ereg_replace("[^0-9]","",$_POST['numRows']);
$cols = mb_ereg_replace("[^0-9]","",$_POST['numCols']);
...
$fontSize = mb_ereg_replace("[^0-9.]","",$_POST['font']);
if($fontSize=="") $fontSize = $fSize;


##### Check language

$language = mb_ereg_replace("[^0-9]","",$_POST['language']);
...
$dbWords = mb_ereg_replace("[^0-9]","",$_POST['dbWords']);
...
##### Pick words from flat file word lists
  ...
  $filename = $pathToWordlists."/".mb_ereg_replace("[@#$%^&*+\\/]","",$_POST['wordlist_lang']);
  ...
  for($a=0;$a<$dbWords;) {
    ...
    if(!$true) {
      ...
      $contents[$_contents[$a]] = mb_ereg_replace("[^".$chars."]", "", trim($contents[$_contents[$a]]));
...
$puzzleTitle = mb_ereg_replace("[@#$%^&*+\\/]","",$_POST['puzzleTitle']);
...
if($_POST['randomWords']!="" && $_POST['randomWords']!=0 && empty($dbWords)) {
  $randomWords = mb_ereg_replace("[^0-9]","",$_POST['randomWords']);
...
$backgroundColor = mb_ereg_replace("[^0-9A-Za-z#]","",$_POST['backgroundColor']);
if($backgroundColor=="") $backgroundColor = $bgColor;
$fontColor = mb_ereg_replace("[^0-9A-Za-z#]","",$_POST['fontColor']);
if($fontColor=="") $fontColor = $fColor;
$highlightColor = mb_ereg_replace("[^0-9A-Za-z#]","",$_POST['highlightColor']);
...
if(isset($_POST['buttonUp']) || isset($_POST['buttonDown']) || isset($_POST['buttonLeft']) || isset($_POST['buttonRight'])) {
  ...
  $t_rows = mb_ereg_replace("[^0-9]","",$t_rows);
  $t_cols = mb_ereg_replace("[^0-9]","",$t_cols);
  ...
  $shift = mb_ereg_replace("[^0-9]","",$_POST['shift']);
  ...
  $n_rows = mb_ereg_replace("[^0-9]","",$_POST['xSize']);
  $n_cols = mb_ereg_replace("[^0-9]","",$_POST['ySize']);
  ...
  $rows = mb_ereg_replace("[^0-9]","",$te[count($te)-2]);
  $cols = mb_ereg_replace("[^0-9]","",$te[count($te)-1]);
  unset($te);
  $code = $_POST['code'];

} else {
  $rows = mb_ereg_replace("[^0-9]","",$_POST['xSize']);
  $cols = mb_ereg_replace("[^0-9]","",$_POST['ySize']);
  if($rows<10) $rows = 10;
  ...
  if ($dh = opendir($dir)) {
    while (($file = readdir($dh)) !== false) {
      if($file!="." && $file!=".." && ereg("_main.php$",$file)) {
        $fileStripped = mb_ereg_replace("_main.php$","",$file);
    ...
    if ($dh = opendir($dir)) {
      while (($file = readdir($dh)) !== false) {
        if($file!="." && $file!=".." && ereg(".txt$",$file)) {
          $fileStripped = mb_ereg_replace("_|.txt$"," ",$file);
          $fileStripped = mb_ereg_replace("[@#$%^&*+\\/]","",$fileStripped);
...
if(isset($_COOKIE['fswordfinder'])) {
  ...
  $rowsSaved = mb_ereg_replace("[^0-9]","",$cookie[0]);
  $colsSaved = mb_ereg_replace("[^0-9]","",$cookie[1]);
  $fontSizeSaved = mb_ereg_replace("[^0-9.]","",$cookie[2]);
  $bgColorSaved = mb_ereg_replace("[^0-9A-Za-z#]","",$cookie[3]);
  $fontColorSaved = mb_ereg_replace("[^0-9A-Za-z#]","",$cookie[4]);
  $highColorSaved = mb_ereg_replace("[^0-9A-Za-z#]","",$cookie[5]);
  $languageSaved = mb_ereg_replace("[^0-9]","",$cookie[6]);
  $borfSaved = $cookie[7];
  $diagSaved = $cookie[8];
  ...
  $randomWordsSaved = mb_ereg_replace("[^0-9]","",$cookie[14]);
...
<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="cl">
  ...
  <input type="hidden" name="backgroundColor" value="<?= mb_ereg_replace("[^0-9A-Za-z#]","",$_POST['backgroundColor']); ?>" />
  <input type="hidden" name="fontColor" value="<?= mb_ereg_replace("[^0-9A-Za-z#]","",$_POST['fontColor']); ?>" />
  <input type="hidden" name="highlightColor" value="<?= mb_ereg_replace("[^0-9A-Za-z#]","",$_POST['highlightColor']); ?>" />
